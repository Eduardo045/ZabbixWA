...
// Truncate string to max length to prevent DoS via large payloads
...
function normalizeAlert(payload) {
  ...
  // Limit to 50 tags max, each name/value truncated
  tags = tags.slice(0, 50).map(t => ({
    tag: trunc(t.tag, 100),
    value: trunc(t.value, 500),
  }));

  const status = (payload.status || payload.event_status || 'PROBLEM').toUpperCase();
  return {
    event_id:     trunc(payload.eventid   || payload.event_id   || '', 64),
    trigger_id:   trunc(payload.triggerid  || payload.trigger_id  || '', 64),
    hostname:     trunc(payload.hostname   || payload.host || payload.HOST || '', 255),
    host_ip:      trunc(payload.hostip     || payload.host_ip || '', 45),
    trigger_name: trunc(payload.triggername || payload.trigger_name || payload.name || payload.eventname || '', 500),
    severity, status,
